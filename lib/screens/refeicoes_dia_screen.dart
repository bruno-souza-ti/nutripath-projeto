...
import '../services/auth_service.dart'; // CORRIGIDO: import adicionado
...
class _RefeicoesDiaScreenState extends State<RefeicoesDiaScreen> {
  ...
  Future<void> _carregarNomeERefeicoes() async {
    final userAuth = await AuthService.getUser();
    if (mounted) {
      setState(() {
        _nomeUsuario = userAuth?['nome'] ?? userAuth?['name'] ?? 'Usuário';
      });
    }
  ...
  int get _totalCalorias => _refeicoes.fold(
    0,
    (sum, r) => sum + ((r['calorias'] as num?)?.toInt() ?? 0),
  );
                  ...
                  onPressed: () async {
                    ...
                    if (desc.isEmpty || cal == null) {
                      ScaffoldMessenger.of(ctx).showSnackBar(
                        const SnackBar(
                          content: Text('Preencha a descrição e as calorias.'),
                        ),
                      );
                      return;
                    }
                    ...
                    // Garantir nome atualizado antes de logar
                    if (_nomeUsuario == 'Usuário') {
                      final u = await AuthService.getUser();
                      _nomeUsuario = u?['nome'] ?? u?['name'] ?? 'Usuário';
                    }
                    await LogService.instance.logarRefeicao(
                      usuarioId: _usuarioId!,
              ...
              children: [
                ...
                IconButton(
                  ...
                  icon: Icon(
                    ...
                    color: _isHoje ? AppTheme.divider : AppTheme.textDark,
                    size: 32,
                  ),
                ),
              ],
  ...
  Widget _buildLista() {
    return ListView.separated(
      ...
      itemBuilder: (_, i) {
        ...
        final corFundo = _tipoColor[tipo] ?? const Color(0xFFF3E5F5);
            ...
            child: Column(
              ...
              children: [
                    ...
                    GestureDetector(
                      onTap: () => showDialog(
                        ...
                        builder: (ctx) => AlertDialog(
                          ...
                          content: const Text('Deseja remover esta refeição?'),
                          actions: [
                            TextButton(
                ...
                Wrap(
                  ...
                  runSpacing: 6,
                  children: [
                    _Chip('🔥 ${r['calorias']} kcal', const Color(0xFFFFF3CD)),
                    if (proteinas != null)
                      _Chip(
